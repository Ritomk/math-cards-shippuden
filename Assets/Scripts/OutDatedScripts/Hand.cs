...
public class Hand : MonoBehaviour
{
    ...
    private GameObject lastHitObject = null; // Pamiêæ ostatniego trafionego obiektu
    ...
    {
        // Obliczanie ró¿nicy po³o¿enia miêdzy centrum a pozycj¹ obiektu
    ...
    {
        float endAngle = startAngle; // Startowy k¹t jest równie¿ k¹tem koñcowym dla 1 karty
        float radius = 5f; // Mo¿esz dostosowaæ promieñ wg potrzeb

        // Jeœli jest wiêcej ni¿ jedna karta, oblicz maksymalny k¹t wychylenia
        if (objectCount > 1)
        {
            endAngle = startAngle + (objectCount - 1) * 10; // Zwiêkszaj k¹t o 10 stopni na ka¿d¹ dodatkow¹ kartê
        }
        ...
        float increment = objectCount > 1 ? angleRange / (objectCount - 1) : 0; // Aby równomiernie rozmieœciæ, unikaj¹c dzielenia przez 0
        ...
        for (int i = 0; i < objectCount; i++)
        {
            ...
            // Obracanie karty w stronê centrum
            cards[i].transform.LookAt(new Vector3(centerPointTransform.position.x, cards[i].transform.position.y, centerPointTransform.position.z));
        }
    }
}
